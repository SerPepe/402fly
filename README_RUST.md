...
The Rust implementation of 402fly provides a comprehensive, type-safe SDK for implementing the X402 payment protocol with Solana blockchain micropayments.
...
- **402fly-core** - Core types, payment processor, and Solana integration
...
[dependencies]
402fly-core = "0.1"
402fly-client = "0.1"  # For client applications
402fly-rocket = "0.1"  # For Rocket servers
# OR
402fly-actix = "0.1"   # For Actix Web servers
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = Fly402AutoClient::new(keypair, None, Some(options));
...
let client = Fly402AutoClient::new(keypair, rpc_url, Some(options));
```
