...
class Fly402Client:
    ...
    def __init__(
        ...
        Args:
            ...
            rpc_url: Solana RPC URL (defaults to devnet with Helius)
            ...
            fee_wallet: Optional fee wallet address (defaults to env or default)
            fee_percentage: Fee percentage as decimal (default: 0.01 for 1%)
        """
        ...
        import os
        default_fee_wallet = fee_wallet or os.getenv(
            "FLY402_FEE_WALLET",
            "2kQGcstuXirRF5HyZvoEmjxdpf9vUpKVrVWcAn9V1gbg"
        )
        ...
        self.fee_wallet = default_fee_wallet
    ...
    async def create_payment(
        ...
        # Calculate total amount needed (payment + fee)
        total_amount_needed = float(pay_amount) * (1.0 + self.fee_percentage)
        ...
        total_amount_decimal = Decimal(str(total_amount_needed))

        if balance_decimal < total_amount_decimal:
            raise InsufficientFundsError(
                f"{total_amount_needed} (payment: {pay_amount} + fee: {float(pay_amount) * self.fee_percentage})",
                str(balance)
            )
        ...
        tx = await self.processor.create_payment_transaction(
            ...
            pay_amount,
            self.wallet_keypair,
            fee_wallet_address=self.fee_wallet,
        ...
        # Note: actual_amount should reflect total paid (payment + fee)
        total_paid = str(total_amount_needed)
        # Note: In Solana, the transaction signature IS the transaction hash
        return PaymentAuthorization(
            payment_id=request.payment_id,
            actual_amount=total_paid,  # Total paid including fee
