...
class SolanaPaymentProcessor:
    ...
    async def create_payment_transaction(
        ...
        """
        Create a Solana transaction for the payment with optional fee

        Args:
            request: Payment request details
            amount: Amount to pay (in token units)
            payer_keypair: Payer's keypair
            fee_wallet_address: Optional fee wallet address (if None, no fee is charged)
            fee_percentage: Fee percentage as decimal (e.g., 0.01 for 1%)
        ...
        try:
            ...
            # Create transfer instruction for recipient (100% of requested amount)
            ...
            # Add fee transfer if fee wallet is provided
            if fee_wallet_address and fee_percentage > 0:
                fee_wallet_pubkey = Pubkey.from_string(fee_wallet_address)
                fee_token_account = get_associated_token_address(
                    fee_wallet_pubkey, token_mint
                )

                # Check if fee wallet's token account exists
                fee_account_info = await self.client.get_account_info(fee_token_account)
                if fee_account_info.value is None:
                    # Create fee wallet's associated token account
                ...
                # Calculate fee amount (1% of payment amount)
                fee_amount = float(amount) * fee_percentage
                fee_amount_in_smallest_unit = int(fee_amount * (10**decimals))
                ...
                fee_transfer_ix = transfer_checked(
                    TransferCheckedParams(
                        ...
                        source=payer_token_account,
                        mint=token_mint,
                        dest=fee_token_account,
                        owner=payer_pubkey,
                        amount=fee_amount_in_smallest_unit,
                        decimals=decimals,
                    )
                )
    ...
    def get_default_rpc_url(self, network: str) -> str:
        ...
        return urls.get(network, "https://evangelina-rc8p3q-fast-mainnet.helius-rpc.com")
