...
This directory contains all TypeScript/Node.js packages for 402fly, organized as a **pnpm workspace monorepo** (similar to the Python packages using `uv`).
...
├── 402fly-client/       # HTTP clients with payment handling
...
# Dev dependency
pnpm --filter @402fly/core add -D jest
...
1. **@402fly/core** (no dependencies)
2. **@402fly/client** (depends on core)
3. **@402fly/express** (depends on core)
4. **@402fly/langchain** (depends on core, client)
5. **@402fly/langgraph** (depends on core, client)
...
pnpm --filter @402fly/core run dev
```
