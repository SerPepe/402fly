...
export interface Fly402Request extends Request {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!paymentAddress || !tokenMint) {
        throw new Error(
          'paymentAddress and tokenMint must be provided either as middleware options or via initFly402()'
        );
      }
      ...
      try {
        ...
        (req as Fly402Request).payment = authorization;
        next();
      } catch (error) {
