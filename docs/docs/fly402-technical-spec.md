# 402fly Technical Specification
...
class PaymentRequiredError(Fly402Error):
    """Raised when 402 response received"""
    code = "PAYMENT_REQUIRED"
    payment_request: PaymentRequest

class PaymentExpiredError(Fly402Error):
    """Payment request has expired"""
    code = "PAYMENT_EXPIRED"

class InsufficientFundsError(Fly402Error):
    """Wallet has insufficient funds"""
    code = "INSUFFICIENT_FUNDS"
    required_amount: str
    available_amount: str

class PaymentVerificationError(Fly402Error):
    """Payment verification failed"""
    code = "PAYMENT_VERIFICATION_FAILED"

class TransactionBroadcastError(Fly402Error):
    """Failed to broadcast transaction"""
    code = "TRANSACTION_BROADCAST_FAILED"

class InvalidPaymentRequestError(Fly402Error):
    """Payment request format is invalid"""
    code = "INVALID_PAYMENT_REQUEST"
```

```typescript
export class Fly402Error extends Error {
  code: string;
  details?: Record<string, any>;
  constructor(message: string, code: string, details?: Record<string, any>);
}

export class PaymentRequiredError extends Fly402Error {
  paymentRequest: PaymentRequest;
  constructor(paymentRequest: PaymentRequest);
}

export class PaymentExpiredError extends Fly402Error {
  constructor(paymentRequest: PaymentRequest);
}

export class InsufficientFundsError extends Fly402Error {
  requiredAmount: string;
  availableAmount: string;
  constructor(required: string, available: string);
}

export class PaymentVerificationError extends Fly402Error {
  constructor(reason: string);
}

export class TransactionBroadcastError extends Fly402Error {
  constructor(reason: string);
}

export class InvalidPaymentRequestError extends Fly402Error {
  constructor(reason: string);
}
```
...
def get_config() -> Fly402Config:
    ...
    if _config is None:
        raise RuntimeError("X402 not initialized. Call init_fly402() first.")
    return _config
```
...
class Fly402Client:
    ...
    Usage:
        client = Fly402Client(wallet_keypair)
...
```python
class Fly402AutoClient:
    """
    Implicit X402 client - automatically handles payment flow
    
    Usage:
        client = Fly402AutoClient(wallet_keypair)
    ...
    def __init__(
        ...
        self.client = Fly402Client(wallet_keypair, rpc_url)
...
export class Fly402Client {
  // Explicit client implementation
}

export class Fly402AutoClient {
...
authors = [
    {name = "402fly Contributors", email = "hello@402fly.org"},
]
readme = "README.md"
license = {text = "MIT"}
requires-python = ">=3.8"
...
logger = logging.getLogger("402fly")
...
This specification provides a comprehensive blueprint for implementing the 402fly library ecosystem. The architecture is modular, allowing for incremental development and easy addition of new framework integrations.
