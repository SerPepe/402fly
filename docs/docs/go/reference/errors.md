...
### Fly402Error (Base Type)

The base error type for all X402 errors.

```go
type Fly402Error struct {
    Message string
    Code    string
    Details map[string]interface{}
}

func (e *Fly402Error) Error() string
```
...
if err != nil {
    if x402Err, ok := err.(*core.Fly402Error); ok {
...
```go
func paymentFlow(ctx context.Context, client *client.Fly402Client, url string) (*http.Response, error) {
...
```go
func autoClientFlow(ctx context.Context, client *client.Fly402AutoClient, url string) (*http.Response, error) {
...
client := client.NewFly402Client(differentKeypair, "", nil, false)
