...
The `402fly-rocket` crate provides integration with the Rocket web framework for protecting endpoints with X402 payment requirements.
...
```rust
rocket::build()
    .manage(config)  // Available in all routes via State<Fly402Config>
    .mount("/", routes![...])
```
