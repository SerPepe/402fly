...
match client.get(url).await {
    ...
    Err(e) => {
        match e {
            Fly402Error::PaymentRequired(msg) => {
                eprintln!("Payment required: {}", msg);
            }
            Fly402Error::InsufficientFunds(msg) => {
                eprintln!("Insufficient funds: {}", msg);
            }
            Fly402Error::PaymentExpired(msg) => {
                eprintln!("Payment expired: {}", msg);
            }
            Fly402Error::Network(msg) => {
...
```rust
async fn fetch_premium_data(client: &Fly402AutoClient) -> Result<String, Box<dyn Error>> {
