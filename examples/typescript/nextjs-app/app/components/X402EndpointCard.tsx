...
import type { PaymentRequestData } from "@402fly/core";
import { generateMockPaymentAuthorization } from "../utils/mock-payment";

interface X402EndpointCardProps {
  title: string;
  description: string;
  endpoint: string;
  method?: string;
...
export default function X402EndpointCard({
  ...
  const handleFetch = async (authHeader?: string) => {
    ...
    try {
      const data = await onFetch(authHeader);
      setResult(data);
    } catch (err) {
  ...
  const handleSimulatePayment = async () => {
    ...
    // 1. Call wallet adapter to sign transaction
    // 2. Create payment transaction
    // 3. Get authorization header
    // 4. Retry with that header

    // For demo, we'll generate a properly formatted mock authorization
    ...
    <div className="border rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      {paymentRequired && (
        <div className="space-y-4">
          <div className="bg-yellow-50 border border-yellow-200 rounded p-4">
            <div className="text-sm font-semibold text-yellow-800 mb-2">💳 Payment Required</div>
            <div className="text-sm text-yellow-700 space-y-1">
              <p><strong>Amount:</strong> {paymentRequired.max_amount_required} {paymentRequired.asset_type}</p>
              <p><strong>Recipient:</strong> <code className="text-xs bg-yellow-100 px-1 py-0.5 rounded">{paymentRequired.payment_address.slice(0, 8)}...{paymentRequired.payment_address.slice(-4)}</code></p>
      ...
      {error && (
        <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded">
          ...
          <pre className="text-xs text-red-700 whitespace-pre-wrap overflow-auto max-h-48">{error}</pre>
        </div>
      )}
    </div>
  );
